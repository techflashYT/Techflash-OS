...
#include <stdio.h>
// 0
void zeroDivHandler(registers_t *regs) {panic("(#DE) Integer Division by Zero", regs);}
// 1, 3
void debugHandler(__attribute__ ((unused)) registers_t *regs) {/* TODO: Start a kernel debugger here */printf("\r\nWould start a kernel debugger here but it's not implemented!\r\n");}
// 2
void NMIHandler(registers_t *regs) {panic("Non-Maskable Interrupt", regs);}
// 4
void overflowHandler(registers_t *regs) {panic("(#OF) Overflow", regs);}
// 5
void boundRangeExceededHandler(registers_t *regs) {panic("(#BR) Bound Range Exceeded", regs);}
// 6
void invalidOpcodeHandler(registers_t *regs) {panic("(#UD) Invalid Opcode (likely a bad jump)", regs);}
/*
...
// 12 (0xC)
void stackSegmentFaultHandler(registers_t *regs) {panic("(#SS) Stack Segment Fault", regs);}
// 13 (0xD)
void generalProtectionFaultHandler(registers_t *regs) {panic("(#GP) General Protection Fault", regs);}
// 14 (0xE)
...
// 17 (0x11)
void alignmentCheckExceptionHandler(registers_t *regs) {panic("(#AC) Alignment Check Exception", regs);}
// 18 (0x12)
void machineCheckExceptionHandler(registers_t *regs) {panic("(#MC) Machine Check Exception", regs);}
// 19 (0x13)
void SSEExceptionHandler(registers_t *regs) {panic("(#XF) SSE Exception", regs);}
// 20 (0x14)
void virtualizationExceptionHandler(registers_t *regs) {panic("(#VE) Virtualization Exception", regs);}
// 21 (0x15)
void controlProtectionExceptionHandler(registers_t *regs) {panic("(#CP) Control Protection Exception", regs);}
// 28 (0x1C)
