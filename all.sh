...
if ! ( command -v x86_64-elf-gcc > /dev/null || command -v x86_64-elf-as > /dev/null || command -v x86_64-elf-ld > /dev/null ); then
	echo "Uh oh!  Looks like you never installed the x86_64-elf-gcc cross compiler!"
	echo "Please follow the steps in the wiki to install it if you want a working build."
	echo -e "\e[1;33m ===> WARNING!!  Attempting to use the system compiler even though it probably won't work!!\e[0m"
	echo "If you're sure you want to do this, type \`yes' here. [yes/no]"
	read -r yesno
	if [ "$yesno" != "yes" ]; then
		echo "User didn't confirm.  Exiting!"
		exit 1
	fi
	echo -e "\e[1;33m ===> WARNING!!  This is entirely unsupported and 99% likely will not work!!  Are you absolutely sure??\e[0m  [yes/no]"
	read -r yesno
	if [ "$yesno" != "yes" ]; then
		echo "User didn't confirm.  Exiting!"
		exit 1
